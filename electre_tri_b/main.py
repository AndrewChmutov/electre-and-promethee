...
def calculate_marginal_concordance_matrix(
    ...
    preference_thresholds[cost_criteria] = -preference_thresholds[cost_criteria]
    indifference_thresholds[cost_criteria] = -indifference_thresholds[cost_criteria]
...
def calculate_marginal_discordance_index[
    ...
    """
    return np.minimum(np.maximum((-diff - p) / (v - p), 0), 1)  # pyright: ignore[reportReturnType]
...
def electre_tri_b(dataset_path: Path) -> None:
    ...
    for assignment_type in assignment.columns:
        ...
        for cls in range(2, -1, -1):
            print(assignment[assignment[assignment_type] == cls].index.tolist())
